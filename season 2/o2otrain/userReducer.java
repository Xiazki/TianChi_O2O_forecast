...
public class userReducer  extends ReducerBase{
	...
	@Override
	  public void reduce(Record key, Iterator<Record> values, TaskContext context)
		...
		int in15day=0;//new add---------------------------
	
		int []every_weekday_lq=new int[7];
		for(int i=0; i<every_weekday_lq.length; ++i)every_weekday_lq[i]=0;
		...
		while(values.hasNext()){
			...
			if (val.getString("date_received")!=null ){
				...
				every_weekday_lq[getWeek(mytime_received)]++;
		...
		result.setBigint("user_all_lq_train",Long.valueOf(received_cnt));
		result.setBigint("user_shang_xun_lq_train",Long.valueOf(shangzhongxia_xun_lq[0]));
		result.setBigint("user_zhong_xun_lq_train", Long.valueOf(shangzhongxia_xun_lq[1]) );
		result.setBigint("user_xia_xun_lq_train", Long.valueOf(shangzhongxia_xun_lq[2]));
		result.setBigint("user_holiday_lq_train", Long.valueOf(holiday_lq));
		result.setBigint("user_workday_lq_train", Long.valueOf(workday_lq));
		//----------
		result.setBigint("user_all_xf_train",Long.valueOf(buy_cnt));
		result.setBigint("user_shang_xun_xf_train",Long.valueOf(shangzhongxia_xun_xf[0]));
		result.setBigint("user_zhong_xun_xf_train", Long.valueOf(shangzhongxia_xun_xf[1]) );
		result.setBigint("user_xia_xun_xf_train", Long.valueOf(shangzhongxia_xun_xf[2]));
		result.setBigint("user_holiday_xf_train", Long.valueOf(holiday_xf));
		result.setBigint("user_workday_xf_train", Long.valueOf(workday_xf));
		//--------------
		result.setBigint("user_all_lq_and_xf_train",Long.valueOf(received_buy_cnt));
		result.setDouble("user_avg_day_train", avg_di(avg_day));
		result.setDouble("user_lq_xf_zhuanhua_train", (double)received_buy_cnt/(received_cnt+1));
		result.setBigint("user_shang_xun_lq_xf_train",Long.valueOf(shangzhongxia_xun_lq_and_xf[0]));
		result.setBigint("user_zhong_xun_lq_xf_train", Long.valueOf(shangzhongxia_xun_lq_and_xf[1]) );
		result.setBigint("user_xia_xun_lq_xf_train", Long.valueOf(shangzhongxia_xun_lq_and_xf[2]));
		//----------------------

		result.setBigint("user_lq_diff_merchant_num_train",Long.valueOf( merchant_list.size()));
		result.setBigint("user_lq_diff_coupon_num_train",Long.valueOf( coupon_list.size()));
		//----------------new add
		result.setDouble("user_lq_avg_dis_train", avg_dis(avg_lq_dis));
		result.setDouble("user_xf_avg_dis_train", avg_dis(avg_xf_dis));
		result.setDouble("user_lq_and_xf_avg_dis_train", avg_dis(avg_lq_and_xf_dis));
		//--------------------------
		result.setBigint("user_lq_max_merchant_train",Long.valueOf( get_max_list(merchant_list)));
		result.setBigint("user_lq_max_coupon_train",Long.valueOf( get_max_list_coupon(coupon_list)));
		
		///////////////new add
		result.setBigint("user_in15day_train",Long.valueOf(in15day));
		result.setBigint("user_lq_max_weekday_lq_train",Long.valueOf( get_max_array_index(every_weekday_lq)));
		result.setDouble("user_lq_avg_discount_rate_float_train", avg_dis(avg_lq_discount_rate_float));
		result.setDouble("user_lq_avg_discount_first_float_train", avg_dis(avg_lq_discount_first_float));
		
		result.setDouble("user_xf_avg_discount_rate_float_train", avg_dis(avg_xf_discount_rate_float));
		result.setDouble("user_xf_avg_discount_first_float_train", avg_dis(avg_xf_discount_first_float));
		
		result.setDouble("user_lq_lq_in15day_bili_train", (double)in15day/(received_cnt+1));
